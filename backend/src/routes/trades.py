...
async def execute(order: dict):
    ...
    req_mode = str(order.get("mode","auto")).lower()
    qty = float(order.get("qty") or 0)
    price = float(order.get("price") or order.get("limit_price") or 0)

    # choose effective mode deterministically
    if live == 1 and kill == 0:
        effective_mode = "live" if req_mode != "shadow" else "shadow"
    else:
        effective_mode = "shadow"

    # log which path we took (structured)
    print({"event":"trade_mode_decision","req_mode":req_mode,"effective_mode":effective_mode,"live":live,"kill":kill})
    ...
    # await log_shadow_trade(symbol, action, qty, price, effective_mode)

    if effective_mode != "live":
        trade_submissions.labels(mode=effective_mode, result="accepted").inc()
        return {"mode":effective_mode,"accepted":True,"symbol":symbol,"qty":qty,"price":price}
    ...
    trade_submissions.labels(mode=effective_mode, result="ok" if ok else "error").inc()
    if not ok:
        raise HTTPException(status_code=502, detail={"error":"broker_reject","broker":result})
    return {"mode":effective_mode,"accepted":True,"broker_order":result}